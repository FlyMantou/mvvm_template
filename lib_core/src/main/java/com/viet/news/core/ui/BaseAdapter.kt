...
abstract class BaseAdapter<T> : RecyclerView.Adapter<BaseAdapter.BaseViewHolder>() {
    ...
    //根据position插入集合
    fun addData(@androidx.annotation.IntRange(from = 0) position: Int, newData: Collection<T>) {
        if (newData == null || newData.isEmpty()) return
    ...
    fun addData(@androidx.annotation.IntRange(from = 0) position: Int, newData: List<T>) {
        if (newData == null || newData.isEmpty()) return
        list.addAll(position, newData)
        compatibilityDataSizeChanged(newData.size)
    }
    ...
    //根据position插入item
    fun setData(@androidx.annotation.IntRange(from = 0) index: Int, data: T) {
        list[index] = data
